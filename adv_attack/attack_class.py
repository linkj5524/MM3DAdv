...
from adv_attack.advcat_tools import *
##
...
class MM3DAdv_ATTACK:
    ...
    def optim_step(self):
        ...
        for epoch in tqdm(range(self.exp_params["optim_epochs"]), desc="Optimizing"):
            ...
            # for backgroud_images, cameras_pose_path in self.optim.train_loader:
            for backgroud_images, cameras_pose_path in tqdm(self.optim.train_loader, desc="Training Loader", leave=True):
                backgroud_images = backgroud_images.to(self.optim.optim_device)
        ...
        save_tensor_root = os.path.join(self.exp_params["experiment_path"], "texture")
        os.makedirs(save_tensor_root, exist_ok=True)
        save_tensor_path = os.path.join(save_tensor_root, "texture_final.pt")
        torch.save(adv_tensor_generate.detach().cpu(), save_tensor_path)
...
class Common_ATTACK:
    ...
    def optim_prepare(self,
        ...
        train_loader=build_from_cfg(self.exp_params["dataloader"])
    ...
    def optim_step(self):
        ...
        for epoch in tqdm(range(self.exp_params["optim_epochs"]), desc="Optimizing"):
            ...
            # for backgroud_images, cameras_pose_path in self.optim.train_loader:
            for index,backgroud_images,mask, cam_relative_pos,cam_relative_rot in tqdm(self.optim.train_loader, desc="Training Loader", leave=True):
                backgroud_images = backgroud_images.to(self.optim.optim_device)
